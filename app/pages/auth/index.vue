    ...
    <div class="min-w-full sm:min-w-[500px] md:min-w-[550px] max-w-full sm:max-w-[550px]">
      <UTabs :items="items" v-model="tab" class="w-full">
        ...
        <template #register="{ item }">
          <UCard @submit.prevent="registerFunc" :ui="card_ui" >
            <template #header>
            ...
            <div class="grid grid-cols-2 gap-5">
              <UFormGroup :label="$t('first_name')" name="first_name" required class="mb-3">
              <UInput v-model="RegistrationForm.first_name" required />
            </UFormGroup>
            <UFormGroup :label="$t('last_name')" name="last_name" required>
              <UInput v-model="RegistrationForm.last_name" required />
            </UFormGroup>
            </div>
